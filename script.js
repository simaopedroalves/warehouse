...
// ==================== BASE DE DADOS DE ARTIGOS (GOOGLE SHEETS) ====================


const COLUNA_ARTIGO = 0;           // <-- 0 = coluna A, 1 = coluna B, etc.

const SHEET_CSV_URL = `https://docs.google.com/spreadsheets/d/e/2PACX-1vQ3_0lwfCxSRbcIi46D1ErB-S6UefFv2U9C155C5HvQ1Ta-uY7XILl0Q9tL_kKYlxfhY0NXCBkS3IxL/pub?output=csv`;

let listaArtigos = [];

async function carregarArtigosDaBaseDeDados() {
  try {
    const response = await fetch(SHEET_CSV_URL);
    if (!response.ok) throw new Error('Erro ao aceder à Google Sheet: ' + response.status);
    const csvText = await response.text();
    listaArtigos = parseCSVParaArtigos(csvText);
    console.log(`${listaArtigos.length} artigos carregados da base de dados.`);
  } catch (error) {
    console.error('Erro ao carregar artigos da base de dados:', error);                
    listaArtigos = [];
  }
}

// Parser simples de CSV que respeita campos entre aspas (com vírgulas dentro)
function parseCSVParaArtigos(csvText) {
  const linhas = csvText.split(/\r?\n/).filter(l => l.trim() !== '');
  const artigos = [];

  for (let i = 1; i < linhas.length; i++) { // i=1 salta o cabeçalho
    const colunas = parseLinhaCSV(linhas[i]);
    const nome = (colunas[COLUNA_ARTIGO] || '').trim();
    if (nome) artigos.push(nome);
  }
...
// ==================== AUTOCOMPLETE DE ARTIGOS NOS TEXTAREAS ====================

document.addEventListener('DOMContentLoaded', () => {
  carregarArtigosDaBaseDeDados();
  document.querySelectorAll('.article-picker').forEach(setupArticlePicker);
});

function setupArticlePicker(pickerEl) {
  ...
  function selecionarArtigo(artigo) {
    artigoSelecionado = artigo;
    searchInput.value = artigo;
    esconderSugestoes();
    qtyInput.focus();
    qtyInput.select();
  }
  ...
  searchInput.addEventListener('keydown', (event) => {
    ...
    if (event.key === 'ArrowDown') {
      ...
      event.preventDefault();
      if (indiceAtivo >= 0 && items[indiceAtivo]) {
        selecionarArtigo(items[indiceAtivo].textContent);
      } else if (items.length === 1) {
        selecionarArtigo(items[0].textContent);
      }
  ...
  function atualizarDestaque(items) {
    ...
    if (items[indiceAtivo]) {
      items[indiceAtivo].classList.add('active');
  ...
  searchInput.addEventListener('blur', () => {
    setTimeout(() => {
      if (searchInput.value !== '' && searchInput.value !== artigoSelecionado) {
        const correspondenciaExata = listaArtigos.find(
          artigo => artigo.toLowerCase() === searchInput.value.trim().toLowerCase()
        );
        if (correspondenciaExata) {
          artigoSelecionado = correspondenciaExata;
          searchInput.value = correspondenciaExata;
        } else {
          searchInput.value = '';
          artigoSelecionado = null;
        }
  ...
  addBtn.addEventListener('click', adicionarArtigoAoTextarea);

  qtyInput.addEventListener('keydown', (event) => {
    if (event.key === 'Enter') {
      event.preventDefault();
      adicionarArtigoAoTextarea();
    }
  });

  function adicionarArtigoAoTextarea() {
    if (!artigoSelecionado) {
      alert('Por favor selecione um artigo válido da lista de sugestões.');
      searchInput.focus();
      return;
    }

    const quantidade = qtyInput.value.trim() || '1';
  ...
  const removeBtn = document.querySelector(`.removeLastArticleBtn[data-target="${targetId}"]`);
  if (removeBtn) {
    removeBtn.addEventListener('click', () => {
      const entradas = textarea.value.split(';').filter(e => e.trim() !== '');
      entradas.pop();
      textarea.value = entradas.length ? entradas.join(';') + ';' : '';
      textarea.dispatchEvent(new Event('input', { bubbles: true }));
    });
  }
}
